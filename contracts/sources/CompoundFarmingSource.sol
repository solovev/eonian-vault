...
contract CompoundFarmingSource is BaseFarmingSource {
    ...
    modifier ensureExchangeRateFresh() {
        require(
            block.number - exchangeRateUpdatedAt <= 1,
            "Exchange rate outdated"
        );
        _;
    }

    function initialize(address _vault, address _compoundToken)
        external
        initializer
    {
        ...
        require(
            vault.getUnderlyingToken() == compoundToken.underlying(),
    ...
    function withdraw(uint256 requiredAmount)
        public
        override
        onlyVault
        _updateExchangeRate
    ...
    function getSuppliedBalance() internal view returns (uint256) {
        ...
        return
            (getCompoundTokenBalance() * exchangeRate) / 10**underlyingDecimals;
    }
